...
> ⚠️ **Work in progress.** The package is usable end-to-end and every chart type renders, but rendering quality on arbitrary user data (long labels, lopsided distributions, asymmetric scales, edge-of-axis annotations) is still being hardened. See [open issues](https://github.com/shalinialisha/dubois-visualization-package/issues) for known rough edges.

## Gallery

These were generated by `examples/stress_test.py` against realistic third-party datasets — not the historical Du Bois data — to validate the package on user-provided inputs.

| | |
|---|---|
| ![Bar — Headcount by function](docs/images/gallery/bar_long_labels.png) | ![Stacked bar — Browser share](docs/images/gallery/stacked_many_segments.png) |
| `bar.bar` — long labels & varied magnitudes | `bar.stacked_bar` — many segments, contrast-aware labels |
| ![Area — Sectors over time](docs/images/gallery/area_5_groups.png) | ![Proportional area — Revenue mix](docs/images/gallery/area_proportional_long_names.png) |
| `area.area` — multi-group time series | `area.proportional_area` — 100% normalized stacks |
| ![Butterfly — Education by census](docs/images/gallery/butterfly_long_labels.png) | ![Comparison — Salary](docs/images/gallery/butterfly_comparison.png) |
| `butterfly.butterfly` — back-to-back comparison | `butterfly.comparison` — dot-plot with value labels |
| ![Spiral — Urban/rural by metro](docs/images/gallery/spiral_long_names.png) | ![Concentric rings — KPIs](docs/images/gallery/spiral_concentric.png) |
| `spiral.spiral` — Plate 11 style proportions | `spiral.concentric_rings` — dashboard-style |
| ![Wrapped — Industry mix](docs/images/gallery/wrapped_long_names.png) | ![Snake — State urbanization](docs/images/gallery/wrapped_snake.png) |
| `wrapped.wrapped_bar` — continuous spiral bar | `wrapped.snake_bar` — proportional row strips |
| ![Icon grid — Smartphone share](docs/images/gallery/pictorial_grid_8groups.png) | ![Pictograph — Likert scale](docs/images/gallery/pictorial_row_7segs.png) |
| `pictorial.icon_grid` — waffle-style proportions | `pictorial.pictograph_row` — single-strip composition |
